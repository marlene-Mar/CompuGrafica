...
* Fecha de entrega: 8 de septiembre del 2024
*/
...
float	hombro = 0.0f, codo = 0.0f, muneca = 0.0f, 
	dedo1 = 0.0f, dedo2 =0.0f, dedo3 = 0.0f, dedo4 =0.0f,
	dedo5 = 0.0f, dedo6= 0.0f, dedo7 = 0.0f, dedo8 = 0.0f;
...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Modelado jerarquico Marlene", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 modelTemp = glm::mat4(1.0f); //Temp controla la información de la posición
		glm::mat4 modelTemp2 = glm::mat4(1.0f); //Temp controla la información de la posición
		glm::mat4 modelTemp3 = glm::mat4(1.0f); //Temp controla la información de la posición
		...
		//En OpenGl el orden de la jerarquía es el mismo en el que se dibuja

		//Model Bicep
		model = glm::rotate(model, glm::radians(hombro), glm::vec3(0.0f, 0.0, 1.0f)); //hombro
		//Coloca el pivote en tal posición que permite la rotación como se desea, antes de dibujar la caja
		...
		//posiión en donde se dibujará el siguiente elemento
		...
		//modelTemp2 matriz auxiliar para regresar al valor de la palma
		...
		model = glm::rotate(model, glm::radians(dedo5), glm::vec3(-0.1f, 0.0f, 1.0f));
		...
		//Model dedo4 A 
		//Se usa ModelTemp2 para que el dedo7 no dependa de los dedos de arriba
		//model = glm::translate(modelTemp2, glm::vec3(0.2f, -0.3f, 0.0f));
 ...
 //Restricciones de rotación y movimiento por teclado para brazo izquierdo
void Inputs(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS) {
		hombro += 0.1f;
		if (hombro > 90.0f) // Restricción de hombro a 90° hacia arriba
			hombro = 90.0f;
	}
	if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS) {
		hombro -= 0.1f;
		if (hombro < -90.0f) // Restricción de hombro a -90° hacia abajo
			hombro = -90.0f;
	}
	//Se considero el movimiento de un codo de brazo izquierdo para observarlo mejor
	if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS) {
		codo += 0.1f;
		if (codo > 0.0f) // Restricción de codo a 0°
			codo = 0.0f;
	}
	if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS) {
		codo -= 0.1f;
		if (codo < -90.0f) // Restricción de codo para que llegue a 90°
			codo = -90.0f;
	}
	if (glfwGetKey(window, GLFW_KEY_M) == GLFW_PRESS) {
		muneca += 0.1f;
		if (muneca > 90.0f) // Restricción de muñeca a 90°
			muneca = 90.0f;
	}
	if (glfwGetKey(window, GLFW_KEY_N) == GLFW_PRESS) {
		muneca -= 0.1f;
		if (muneca < -90.0f) // Restricción de codo a -90°
			muneca = -90.0f;
	}
	...
	if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) {
		...
		dedo3 -= 0.1f;
		if (dedo3 < -20.0f) dedo3 = -20.0f;
		dedo5 -= 0.1f;	
		if (dedo5 < -20.0f) dedo5 = -20.0f;
	}
